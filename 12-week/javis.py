...
try:
    ...
    print('SpeechRecognition 라이브러리가 설치되어 있지 않습니다.')
    print('터미널에서 다음 명령어를 실행하세요: pip install SpeechRecognition')
    exit()
...
def convert_audio_to_text():
    ...
    try:
        file_number = int(input('변환할 파일 번호를 선택하세요: '))
        selected_file = wav_files[file_number - 1]
    except (ValueError, IndexError):
        print('올바른 번호를 입력하세요.')
        return

    wav_path = os.path.join(RECORDS_DIR, selected_file)
...
def search_text_from_csv():
    ...
    for file_name in csv_files:
        csv_path = os.path.join(RECORDS_DIR, file_name)
